...
def export(song: Path, take_id: str, gains: dict, with_video: bool,
    ...
    if start_s is not None:
        a = max(a, int(round(start_s * sr)))
    if end_s is not None:
        b = min(b, int(round(end_s * sr)))
    if b - a < sr // 2:
        raise TakeError("The chosen range doesn't overlap the take" if start_s is not None or end_s is not None
                        else "This take doesn't overlap the song")
